...
function parseDate(dateStr, format = "mm/dd/yyyy") {
  const result = getDateParts(dateStr, format);
...
function isValidDate(dateStr, format = "yyyy-mm-dd") {
  try {
    if (typeof dateStr !== "string" || format.length < 8) {
      return false;
    }
    ...
    console.debug(
      "AuroDatepickerUtilities | isValidDate: Error validating date",
      error,
    );
    return false;
  }
 ...
 */
function isISOFormattedDate(dateStr) {
  ...
  try {
    ...
    console.debug(
      "AuroDatepickerUtilities | isISOFormattedDate: Error validating ISO format",
      error,
    );
    return false;
  }
...
function stringToDateInstance(dateStr, format = "yyyy-mm-dd") {
  if (typeof dateStr !== "string") {
    console.debug(
      "AuroDatepickerUtilities | stringToDate: Date must be a string",
    );
    return null;
  }
  ...
  const { month, day, year } = parseDate(normalizedDateStr, format);
