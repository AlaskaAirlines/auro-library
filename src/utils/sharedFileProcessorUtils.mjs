...
import { AuroFileHandler } from "../../scripts/utils/auroFileHandler.mjs";
...
export function generateWCGeneratorUrl(branchOrTag, fileNameWithPath) {
  const baseRepoUrl =
    "https://raw.githubusercontent.com/AlaskaAirlines/WC-Generator";
 ...
 * @return {Promise<void>}
 */
export async function runMarkdownMagicOnFile(
  input,
  output,
  extraMdMagicConfig = {},
) {
...
export async function processContentForFile(config) {
  ...
  // Helper vars
  const derivedInputPath =
    typeof rawInput === "string" ? rawInput : rawInput.fileName;
